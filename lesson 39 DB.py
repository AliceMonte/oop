...
conn.commit()
curs.execute("SELECT * FROM Konference")
vyb = curs.fetchall()
print(vyb)
print('Выборка потребности участников (ФИО) в гостинице:')
curs.execute("SELECT full_name FROM Spravocnik WHERE hotel_need = 'yes'")
vyb1 = curs.fetchall()
print(vyb1)
curs.close()
conn.close()
